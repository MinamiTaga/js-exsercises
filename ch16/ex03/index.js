...
function encrypt64(text, key) {
  ...
  // 暗号化とBase64エンコード
  // ここを埋める
  const cipher = crypto.createCipheriv("aes-256-cbc", key, iv); // 暗号化するためのCipherオブジェクト
...
async function readKey() {
  ...
  return Buffer.from(keyObj.key, "base64");
}
...
function decrypt64(data, key) {
  ...
  const decipher = crypto.createDecipheriv("aes-256-cbc", key, iv); // 復号用のDecipherオブジェクト
