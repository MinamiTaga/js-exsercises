...
onmessage = function (message) {
  // まず、受け取ったメッセージを展開する。
  // - tile は、width と height というプロパティを持つオブジェクト。
  ...
  // 次に、ピクセルの矩形配列を表す ImageData オブジェクトを作成し、
  // 内部の ArrayBuffer を取得し、そのバッファに対して型付き配列の
  // ビューを作成する。このようにすることで、各ピクセルを 4 バイト
  ...
  // これから計算を開始する。この計算には、入れ子になった 3 つのループがある。
  // 外側の 2 つのループは、ピクセルの行と列をループする。内側のループは、
  // 各ピクセルが「脱出」するかどうかを調べるために反復する。各ループの
  // 変数は以下の通り。
  // - row と column は、ピクセル座標を表す整数。
  // - x と y は、各ピクセルの複素点を表す: x + yi。
  // - index は、現在のピクセルの反復配列におけるインデックス。
  // - max と min は、矩形内の任意のピクセルについて、これまでの反復回数の
  // 最大値と最小値を記憶する。
  let index = 0,
    max = 0,
    min = maxIterations;
  for (let row = 0, y = y0; row < height; row++, y += perPixel) {
    for (let column = 0, x = x0; column < width; column++, x += perPixel) {
      // 各ピクセルについて、まず複素数 c = x+yi を求める。
      // そして、次の再帰式に基づいて、複素数 z(n+1) を繰り返し
      // 計算する:
      // z(0) = c
      // z(n+1) = z(n)^2 + c
      // もし|z(n)|（z(n) の大きさ）が 2 より大きければ、そのピクセルは
      // 集合には含まれないので、n 回の反復で停止する。
      let n; // これまでの反復回数。
      let r = x,
        i = y; // z(0) を c に設定して開始する。
      for (n = 0; n < maxIterations; n++) {
        let rr = r * r,
          ii = i * i; // z(n) の 2 つの部分を自乗する。
        if (rr + ii > 4) {
          // |z(n)|^2 が 4 より大きければ、
          break; // 脱出したので、反復を停止する。
        }
        i = 2 * r * i + y; // z(n+1) の虚数部を計算する。
        r = rr - ii + x; // そして z(n+1) の実数部を計算する。
      }
