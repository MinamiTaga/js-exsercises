...
class Tile {
  constructor(x, y, width, height) {
    ...
    this.y = y; // プロパティは、大きな
    this.width = width; // 矩形の中での Tile の
 ...
 * その計算結果を 1 つのメッセージとして送信する。
 *
 * WokerPool と実行する作業を表すメッセージがあれば、メッセージを引数
 * として addWork() を呼び出すだけ。現在アイドル状態の Worker オブジェクトが
 * あれば、メッセージはすぐにそのワーカーに送信される。アイドル状態の
 * Woker が存在しない場合、メッセージはキューに入れられ、ワーカーが利用
 * 可能になったときに送信される。
 *
 * addWork() は Promise を返す。この Promise はワーカーから受け取ったメッセージで
...
class WorkerPool {
  ...
  _workerDone(worker, error, response) {
    ...
    // 最後に、ワーカーに関連する Promise を解決させるか、失敗させる。
    error === null ? resolver(response) : rejector(error);
  }
  // このメソッドは、ワーカープールに仕事を追加し、Promise を返す。
  // この Promise は、仕事が完了したときに、ワーカーの応答で解決する。
 ...
 * cx と cy プロパティは、画像の中心となる複素平面上の点を示す。perPixel
 ...
 * このオブジェクトを使って、ブックマークされた URL やシェアされた URL から
 * 必要な状態を読み取る。
 */
class PageState {
  ...
  static fromURL(url) {
    ...
    // 有効な値を取得できた場合は PageSate オブジェクトを返す。そうでない場合は null。
    return isNaN(s.cx) ||
      isNaN(s.cy) ||
      isNaN(s.perPixel) ||
...
class MandelbrotCanvas {
  constructor(canvas) {
    ...
    // canvas の大きさを設定して、それを覆う tile の配列を取得する。
    this.setSize();
    // そして、マンデルブロ集合を canvas に描画する。
    this.render();
  ...
  // 加える。第 1 引数がオブジェクトの場合は、そのオブジェクトのプロパティを
  // 状態オブジェクトにコピーするだけ。省略可能な第 2 引数が false の場合、
  // 新しい状態は保存されない（popstate イベントに応答して setState を
  // 呼び出すときにはこのようにする）。
  setState(f, save = true) {
    ...
    this.render();
    // 通常は新しい状態を保存する。ただし、第 2 引数に false が指定されて
  ...
  // 一部を canvas に描画する。このメソッドは、コンストラクタや、状態が
  // 変化したときの setState() や、canvas の大きさが変化したときの resize
  // イベントハンドラから呼び出される。
  render() {
    ...
    // 反復回数を含む ImagaData オブジェクト、また、その Tile の最小および
    // 最大反復回数が含まれている。
    this.pendingRender = Promise.all(promises)
      .then((responses) => {
        // まず、すべての Tile に対する全体の反復回数の最大値と最小値を
        ...
        // 変換する方法が必要になる。すべてのピクセルの反復回数は min と max の
        ...
        // 最後に、putImageData() を使って、すべての imageData オブジェクトを、
        // canvas の対応する tile に描画する（ただし、最初に、pointerdown
  ...
  // サイズ変更イベントを受信してから 200ms 経過するまで、サイズ変更の処理を
  // 延期する。
  handleResize(event) {
    ...
    this.resizeTimer = setTimeout(() => {
      ...
      this.setSize(); // canvas と tile のサイズを変更する。
      this.render(); // 新しいサイズで再描画する。
    }, 200);
  }
  // ユーザがキーを押下した場合、このイベントハンドラが呼び出される。
  // 押下されたキーに応じて setState() を呼び出し、setState() は
  ...
  handlePointer(event) {
    ...
    // ユーザがジェスチャーを開始すると、それに続く pointermove と
